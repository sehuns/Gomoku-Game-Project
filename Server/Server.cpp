...
// 특정 클라이언트에게 "엔터" 라는 접속확인 메시지 보내주는 함수
...
void Server::ServerThread(Client *client) {
	...
	while (true) {
		...
		if ((size = recv(client->getClientSocket(), received, 256, NULL)) > 0) {
			...
			vector<string> tokens = util.getTokens(receivedString, ']'); // 토큰을 ] 기준으로 분리해서
				if (receivedString.find("[Enter]") != -1) { //엔터 라면 접속 가능한것이니까
				...
				// 방 번호를 계속 기억하고 있기 때문에 그 방에 속한 사람만 ..!
