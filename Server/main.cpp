				...
				for (int i = 0; i < connections.size(); i++) {
					...
					if (connections[i].getClientSocket() == client->getClientSocket()) {
						...
						if (clientCount >= 2) {
							ZeroMemory(sent, 256);
							sprintf(sent, "%s", "[Full]");
							send(connections[i].getClientSocket(), sent, 256, 0);  //여기가 핵심이고..
							break;
						}
...
int main() {
	...
	while (true) {
		...
		if (clientSocket = accept(serverSocket, (SOCKADDR*)&serverAddress, &addressLength)) {
			...
			CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)ServerThread, (LPVOID)client, NULL, NULL); // 접속되는 클라이언트들은 쓰레드처리해야함.
